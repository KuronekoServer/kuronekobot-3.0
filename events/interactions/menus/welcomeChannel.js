...
module.exports = async(interaction, client) => {
    ...
    if (interaction.values[0] === "welcome_channel") {
        ...
        if (!data) {
            msg.edit("参加メッセージを送る**チャンネルID**を送ってください！")
            ...
            collector.on('collect', async(collected, returnValue) => {
                ...
                return msg.edit(`参加メッセージチャンネルを ${interaction.guild.channels.cache.get(channelID)} に設定しました。`)
            })
            ...
            return msg.edit(`参加メッセージを無効にしました！`)
        }
    }
}