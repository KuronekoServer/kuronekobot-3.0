...
module.exports = async(interaction, client) => {
    ...
    if (interaction.values[0] === "leave_channel") {
        ...
        if (!data) {
            msg.edit("退出メッセージを送る**チャンネルID**を送ってください！")
            ...
            collector.on('collect', async(collected, returnValue) => {
                ...
                if (!channel) return msg.edit("無効なチャンネルIDです！")
                ...
                return msg.edit(`退出メッセージチャンネルを ${interaction.guild.channels.cache.get(channelID)} に設定しました。`)
            })
            ...
            return msg.edit(`退出メッセージを無効にしました！`)
        }
    }
}